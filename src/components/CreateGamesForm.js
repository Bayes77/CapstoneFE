...
function CreateGamesForm({ obj = initialState }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (obj.firebaseKey) {
      updateGames(formInput).then(() => router.push(`/game/${obj.firebaseKey}`));
    } else {
  ...
  return (
    <Form onSubmit={handleSubmit} className="text-black">
      ...
      {/* *Email INPUT 
      <FloatingLabel controlId="floatingInput4" label="Players" className="mb-3">
        <Form.Control type="text" placeholder="# of Players" name="Players" value={formInput.numberOfPLayers} onChange={handleChange} required />
      </FloatingLabel> */}
